...
@Form.create()
export default class %(title)s extends PureComponent {
  constructor(props) {
    super(props);
    this.columns = [
      {
        title: '名称',
        dataIndex: 'name',
        render: (text, record) => <Link to={`/asset/%(modelName)s/${record.id}/detail`}>{text}</Link>,
      },
      {
        title: '类型',
